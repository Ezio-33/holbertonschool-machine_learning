...
def spheric_generator(nb_points, dim):
    ...
    return u / tf.reshape(tf.sqrt(tf.reduce_sum(tf.square(u),
                          axis=[1]) + 10**-8), [nb_points, 1])
...
def cloud_in_the_corner(C, sigma, S):
    return tf.convert_to_tensor(
        cloud_in_the_corner_numpy(
            C, sigma, S), dtype="float32")
...
def verif_squared_dist_barycenter_to_center(N, sigma, S):
    ...
    print(
        f"""Among 10000 clouds of {S} points, the distribution of the squared distance
...
def fully_connected_GenDiscr(gen_shape, real_examples, latent_type="normal"):
    ...
    for i in range(2, len(gen_shape) - 1):
        hidden = keras.layers.Dense(
            gen_shape[-1 * i], activation='tanh')(hidden)
...
def example_fully_connected_GAN(
        chosen_type,
        real_examples,
        gen_shape,
        epochs,
...
set_seeds(0)
Gs = [
    example_fully_connected_GAN(
        chosen_type,
        real,
        shape,
        100,
        learning_rate=lr) for chosen_type in [
            Simple_GAN,
            WGAN_clip,
        WGAN_GP]]
...
fig.suptitle(
    "expected msdcc : {:.4f} , expected dbc : {:.4f}".format(
        expected_msdcc,
        expected_dbc))
plt.tight_layout()

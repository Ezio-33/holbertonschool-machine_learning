#!/usr/bin/env python3
"""
Module implémentant la mise à jour d'un processus gaussien
"""
...
class GaussianProcess:
    """
    Classe pour les mises à jour d'un processus gaussien avec noyau RBF
    ...
    def update(self, X_new, Y_new):
        """
        Met à jour le processus avec de nouvelles observations

        Args:
            X_new (np.ndarray): Nouveau point d'entrée (forme (1,))
            Y_new (np.ndarray): Nouvelle sortie observée (forme (1,))
        """
        # Conversion en format 2D
        ...
        # Calcul des nouvelles covariances
        k_ss = self.kernel(X_new, X_new)
        k_s = self.kernel(self.X[:-1], X_new)

        # Construction de la nouvelle matrice K
        new_K = np.block([[self.K, k_s],
                         [k_s.T, k_ss]])

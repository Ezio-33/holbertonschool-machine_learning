...
def expectation_maximization(X, k, iterations=1000, tol=1e-5, verbose=False):
    ...
    Paramètres:
    X : np.ndarray
        Tableau de données de forme (n, d) où n est le nombre d'exemples et d est la dimension.
    k : int
        Nombre de mélanges (clusters).
    iterations : int, facultatif
        Nombre maximum d'itérations. Par défaut à 1000.
    tol : float, facultatif
        Tolérance pour la convergence de la log vraisemblance. Par défaut à 1e-5.
    verbose : bool, facultatif
        Si True, affiche la log vraisemblance tous les 10 itérations. Par défaut à False.

...
class Normal:
    ...
    def __init__(self, data=None, mean=0., stddev=1.):
        ...
        Args:
            ...
            mean (float, optional): Moyenne de la distribution
        ...
        """
        if data is None:
            # Si aucune donnée n'est fournie, utiliser les mean et stddev spécifiés
            if stddev <= 0:
                raise ValueError("stddev must be a positive value")
            self.mean = float(mean)
            self.stddev = float(stddev)
        else:
            # Si des données sont fournies, calculer mean et stddev à partir des données

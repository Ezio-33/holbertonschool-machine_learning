...
class Exponential:
    ...
    def __init__(self, data=None, lambtha=1.):
        ...
        Args:
            data (list, optional): Liste de données pour
            estimer la distribution lambtha (float, optional):
            Taux d'occurrence des événements
        ...
        if data is None:
            ...
            # calculer lambtha à partir des données
            if not isinstance(data, list):
                raise TypeError("data must be a list")
            if len(data) < 2:
                raise ValueError("data must contain multiple values")

            # Pour une distribution exponentielle, lambtha
            # est l'inverse de la moyenne

...
def cumulative_bleu(references, sentence, n):
    ...
    for i in range(1, n + 1):
        ...
        ngrams_sentence = [tuple(sentence[j:j + i]) for j in range(len(sentence) - i + 1)]
        ...
        for ref in references:
            ...
            for ngram in ref_counter:
                max_ref_counter[ngram] = max(max_ref_counter[ngram], ref_counter[ngram])

        clipped_count = sum(
            min(count, max_ref_counter[ngram]) for ngram, count in sentence_counter.items()
        )
    ...
    # Brevity Penalty
    closest_ref_len = len(min(references, key=lambda ref: abs(len(ref) - len(sentence))))

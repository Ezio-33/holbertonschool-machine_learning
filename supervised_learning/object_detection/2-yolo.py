...
class Yolo():
    """
    Classe Yolo qui utilise l'algorithme Yolo v3 pour effectuer la
    détection d'objets
    """
    ...
    def __init__(self, model_path, classes_path, class_t, nms_t, anchors):
        ...
        Arguments:
         ...
         - classes_path est le chemin où la liste des noms de classes utilisés
         pour
                le modèle Darknet, listés dans l'ordre des index, peut être
                trouvée
        ...
        Attributs d'instance publics :
         ...
         - class_t : le seuil de score de boîte pour l'étape de filtrage
         initiale
    ...
    def process_outputs(self, outputs, image_size):
        ...
        Arguments:
         ...
         - image_size est un numpy.ndarray contenant la taille originale
         de l'image
        ...
        Renvoie :
         ...
         - boxes : une liste de numpy.ndarrays de forme
                ...
                contenant les boîtes de délimitation traitées pour
                chaque sortie :
                * 4 => (x1, y1, x2, y2)
                * (x1, y1, x2, y2) devraient représenter la boîte de
                délimitation
                        relative à l'image originale
         - box_confidences : une liste de numpy.ndarrays de forme
                (grid_height, grid_width, anchor_boxes, 1)
                contenant les confiances des boîtes pour chaque sortie,
                respectivement
    ...
    def filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        Arguments:
         - boxes : une liste de numpy.ndarrays de forme
                 (grid_height, grid_width, anchor_boxes, 4)
                contenant les boîtes de délimitation traitée
                pour chaque sortie
         - box_confidences : une liste de numpy.ndarrays de forme
                 (grid_height, grid_width, anchor_boxes, 1)
                contenant les confiances des boîtes traitées pour
                chaque sortie
         - box_class_probs : une liste de numpy.ndarrays de forme
                 (grid_height, grid_width, anchor_boxes, classes)
                contenant les probabilités de classe des boîtes
                traitées pour chaque sortie
        Renvoie :
         ...
         * box_classes : un numpy.ndarray de forme (?,) contenant
                le numéro de classe que chaque boîte dans filtered_boxes
                prédit, respectivement
         * box_scores : un numpy.ndarray de forme (?) contenant
                les scores des boîtes pour chaque boîte dans filtered_boxes,
                respectivement
        """

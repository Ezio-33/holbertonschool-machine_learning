...
# Calcule et définit les matrice de Gram pour les sorties des couches de style
    ...
    def variational_cost(generated_image):
        """
        Calcule le coût variationnel pour lisser l'image

        Args:
            generated_image: Tenseur TF de forme (1, H, W, 3)

        Returns:
            Coût variationnel (somme des différences carrées)
        """
        # Différences horizontales et verticales entre pixels adjacents
        diff_y = generated_image[:, 1:, :, :] - generated_image[:, :-1, :, :]
        diff_x = generated_image[:, :, 1:, :] - generated_image[:, :, :-1, :]

        # Somme des carrés des différences
        return (tf.reduce_sum(tf.square(diff_x)) +
                tf.reduce_sum(tf.square(diff_y)))

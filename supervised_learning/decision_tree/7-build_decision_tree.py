#!/usr/bin/env python3
""" Tâche 1 : 1. Nombre de nœuds/feuilles dans un arbre de décision"""
import numpy as np
...
def left_child_add_prefix(text):
    ...
    indiquer qu'il s'agit de l'enfant gauche dans la structure de l'arbre.
    ...
    Retourne:
    str
        Le texte avec le préfixe d'enfant gauche ajouté à chaque ligne.
    """
    lines = text.split("\n")
    new_text = "    +--" + lines[0] + "\n"
    for x in lines[1:]:
...
def right_child_add_prefix(text):
    """
    Ajoute un préfixe à chaque ligne du texte pour
    indiquer qu'il s'agit de l'enfant droit dans la structure de l'arbre.
    ...
    Retourne:
    str
        Le texte avec le préfixe d'enfant droit ajouté à chaque ligne.
    """
    lines = text.split("\n")
    new_text = "    +--" + lines[0] + "\n"
    for x in lines[1:]:
...
class Node:
    ...
    sub_population : any
        L'ensemble des données à ce nœud.
    depth : int
    ...
    def __init__(self, feature=None, threshold=None,
        ...
        feature : int ou None, optionnel
            La caractéristique utilisée pour diviser les données (par défaut,
            None).
    ...
    def count_nodes_below(self, only_leaves=False):
        ...
        only_leaves : bool, optionnel
            Si True, compte uniquement les feuilles (par défaut, False).
    ...
    def __str__(self):
        """
        Fournit une représentation en chaîne du nœud, incluant ses enfants.

        Retourne:
        str
            Une chaîne formatée représentant le sous-arbre enraciné
            à ce nœud.
        """
        if self.is_root:
            result = (
                f"racine [feature={self.feature}, threshold={self.threshold}]\n"
            )
        else:
            result = (
                f"nœud [feature={self.feature}, threshold={self.threshold}]\n"
            )

        # Ajouter l'enfant gauche avec préfixe
        if self.left_child:
            left_str = self.left_child.__str__()
            result += left_child_add_prefix(left_str)

        # Ajouter l'enfant droit avec préfixe
        if self.right_child:
    ...
    def update_bounds_below(self):
        """
        Calcule et met à jour récursivement les dictionnaires
        de bornes inférieures et supérieures
        pour chaque nœud et ses enfants basés sur les seuils des
        caractéristiques.
        """
        if self.is_root:
            # Initialiser les bornes à la racine
            self.upper = {0: np.inf}
            self.lower = {0: -np.inf}

        # Calculer les bornes pour les enfants
        if self.left_child:
            self.left_child.lower = self.lower.copy()
            self.left_child.upper = self.upper.copy()
            # Mettre à jour la borne supérieure pour la caractéristique
            self.left_child.lower[self.feature] = self.threshold

        if self.right_child:
            self.right_child.lower = self.lower.copy()
            self.right_child.upper = self.upper.copy()
            # Mettre à jour la borne inférieure pour la caractéristique
            self.right_child.upper[self.feature] = self.threshold

        # Mettre à jour récursivement les bornes pour les enfants
    ...
    def update_indicator(self):
        ...
        basé sur les bornes inférieures et supérieures.
        """
        ...
        def is_large_enough(x):
            ...
            np.ndarray
                Un tableau 1D de valeurs booléennes
                indiquant si chaque individu satisfait la condition.
            """
...
class Leaf(Node):
    """
    Une classe représentant un nœud feuille dans un arbre de
    décision, héritant de Node.
    ...
    def __str__(self):
        """
        Retourne une représentation en chaîne du nœud feuille.

        Retourne:
        str
            La représentation en chaîne du nœud feuille.
        """
        return f"-> feuille [value={self.value}]"

    def get_leaves_below(self):
        """
        Retourne le nœud feuille lui-même dans une liste.
        """
        return [self]

    def update_bounds_below(self):
        """
        Les nœuds feuilles héritent des bornes de leurs
...
class Decision_Tree():
    ...
    predict : any
        Méthode pour prédire la valeur cible pour un ensemble
        de caractéristiques.
    ...
    def __init__(self, max_depth=10, min_pop=1, seed=0,
        ...
        split_criterion : str, optionnel
            Le critère utilisé pour diviser les nœuds (par défaut,
            "random").
    ...
    def __str__(self):
        """
        Retourne une représentation en chaîne de l'arbre de
        décision.
    ...
    def get_leaves(self):
        """
        Retourne la liste de tous les nœuds feuilles dans l'arbre
        de décision.
    ...
    def update_bounds(self):
        """
        Met à jour les bornes pour tous les nœuds dans l'arbre
        de décision.
        """
    ...
    def update_predict(self):
        """
        Met à jour la fonction predict pour des prédictions
        en lot efficaces.
        """
        ...
        # Mettre à jour l'indicateur pour chaque feuille et stocker
        # sa contribution
        for leaf in leaves:
            leaf.update_indicator()

        # Définir la fonction predict efficace
    ...
    def random_split_criterion(self, node):
        ...
        Paramètres
        node : Node
            Le nœud pour lequel le critère de division est
            déterminé.

        Retourne
        tuple
            Un tuple contenant l'indice de la caractéristique et
            la valeur seuil.
        """
        diff = 0
        while diff == 0:
    ...
    def fit(self, explanatory, target, verbose=0):
        """
        Ajuste l'arbre de décision aux données explicatives et
        cibles fournies.
        ...
        verbose : int, optionnel
            ...
            (par défaut, 0).
        """
        # Définir la méthode du critère de division
        ...
        - Nombre de feuilles        : {self.count_nodes(only_leaves=True)}""")
            print(f"    - Précision sur les données "
                  f"d'entraînement : {self.accuracy(self.explanatory, self.target)}")
    ...
    def np_extrema(self, arr):
        ...
        Retourne:
        tuple
            Un tuple contenant les valeurs minimale et maximale du
            tableau.
        """
    ...
    def get_node_child(self, node, sub_population):
        ...
        Retourne
        Node
            Le nœud enfant non-feuille créé.
        """
        n = Node()
    ...
    def accuracy(self, test_explanatory, test_target):
        """
        Calcule la précision de l'arbre de décision sur les
        données de test.
        ...
        Retourne:
        float
            La précision de l'arbre de décision sur les données de
            test.
        """
        return np.sum(
